...
class CollectController extends Controller {
    ...
    public function index() {
        ...
        $count = $collectModel->where(['agentid'=>session(C("AGENT_USER_AUTH_DATA"))['agentid']])->count();// 查询满足要求的总记录数

        $Page = new \Think\Page($count, 20);// 实例化分页类 传入总记录数和每页显示的记录数(25)
        $Page->setConfig('theme', C("PAGE_THEME"));
        $show = $Page->show();// 分页显示输出
        $lists = $collectModel->index($where, $Page);
    ...
    public function collect(){
        $collectModel = new BAgentCollectModel();
        $ret = $collectModel->addRecord(I('productid'));
        $this->processResult($ret);
    }
    ...
    public function unCollect(){
        $collectModel = new BAgentCollectModel();
        $ret = $collectModel->deleteRecord(I('productid'));
        $this->processResult($ret);
    }
