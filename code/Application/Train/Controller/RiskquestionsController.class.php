...
use Train\Model\ARiskquestionsModel;
class RiskquestionsController extends Controller {
    ...
    public function __construct()
    {
        ...
        if($this->instance == NULL) {
            $this->instance = new ARiskquestionsModel;
        }
    }
    ...
    public function add()
    {
        ...
        if($question == '')
        {
            return $this->error('问题不能为空！', U('Train/Riskquestions/index'));
        }
        if($mytext == '')
        {
            return $this->error('答案不能为空！', U('Train/Riskquestions/index'));
        }
        $data['answerlist'] = json_encode($mytext);
        $data['scorelist'] = json_encode($textscore);
        $data['createtime'] = strtotime(date('Y-m-d H:i:s', time()));
        $data['type'] = 1;//1:单选，2：双选，3：多选
        $result = $this->instance->add($data);
        if($result)
        {
            return $this->success('题目添加成功！', U('Train/Riskquestions/index'));
        }
    }
    ...
    public function delete()
    {
        ...
        if($result)
        {
            return $this->success('题目删除成功！',U('Train/Riskquestions/index'));
        }
    }
    ...
    public function edit()
    {
        ...
        if($result)
        {
            return $this->success('题目修改成功！', U('Train/Riskquestions/index'));
        }
    }
    /**
     * 风险承受测试题库批量删除题目
    ...
    public function batchDelete() {
        $ids = I('post.request');
        if(empty($ids)){
            redirect(U('Train/Riskquestions/index'));
        }
    ...
    public function start()
    {
        ...
        if($result)
        {
            return $this->success('启动成功！', U('Train/Riskquestions/index'));
        }
        else
        {
            return $this->success('启动失败！', U('Train/Riskquestions/index'));
        }
    }
}