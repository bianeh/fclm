...
//生成验证码
function MakeCode($expire = 300){
    $number = mt_rand(110001, 999999);
    session('verfiyCode', ['code'=>$number,'mktime'=>time(),'expire'=>$expire]);
    return $number;
}

//认证验证码
function verfiyCode($code){
    if(session('verfiyCode.code') === $code && (time()-session('verfiyCode.mktime') <= session('verfiyCode.expire'))){
        return true;
    }  else {
        return false;
    }
