...
class RbacIFA {
    ...
    static public function authenticate($map,$model='') {
        if(empty($model)) $model =  C('AGENT_USER_AUTH_MODEL');
        //使用给定的Map进行认证
    ...
    static function saveAccessList($authId=null) {
        if(null===$authId)   $authId = $_SESSION[C('AGENT_USER_AUTH_KEY')];
        // 如果使用普通权限模式，保存当前用户的访问权限列表
        // 对管理员开发所有权限
        if(C('AGENT_USER_AUTH_TYPE') !=2 && !$_SESSION[C('AGENT_ADMIN_AUTH_KEY')] )
            $_SESSION['_AGENT_ACCESS_LIST']	=	self::getAccessList($authId);
        return ;
    }
    ...
    static function checkAccess() {
        ...
        if( C('AGENT_USER_AUTH_ON') ){
			$_module	=	array();
			$_action	=	array();
            if("" != C('AGENT_REQUIRE_AUTH_MODULE')) {
                //需要认证的模块
                $_module['yes'] = explode(',',strtoupper(C('AGENT_REQUIRE_AUTH_MODULE')));
            }else {
                //无需认证的模块
                $_module['no'] = explode(',',strtoupper(C('AGENT_NOT_AUTH_MODULE')));
            }
            //检查当前模块是否需要认证
            if((!empty($_module['no']) && !in_array(strtoupper(CONTROLLER_NAME),$_module['no'])) || (!empty($_module['yes']) && in_array(strtoupper(CONTROLLER_NAME),$_module['yes']))) {
				if("" != C('AGENT_REQUIRE_AUTH_ACTION')) {
					//需要认证的操作
					$_action['yes'] = explode(',',strtoupper(C('AGENT_REQUIRE_AUTH_ACTION')));
				}else {
					//无需认证的操作
					$_action['no'] = explode(',',strtoupper(C('AGENT_NOT_AUTH_ACTION')));
				}
				//检查当前操作是否需要认证
	...
	static public function checkLogin() {
        ...
        if(self::checkAccess()) {
            //检查认证识别号
            if(!$_SESSION[C('AGENT_USER_AUTH_KEY')]) {
                if(C('AGENT_GUEST_AUTH_ON')) {
                    // 开启游客授权访问
                    if(!isset($_SESSION['_AGENT_ACCESS_LIST']))
                        // 保存游客权限
                        self::saveAccessList(C('AGENT_GUEST_AUTH_ID'));
                }else{
                    // 禁止游客访问跳转到认证网关
                    redirect(PHP_FILE.C('AGENT_USER_AUTH_GATEWAY'));
                }
            }
        }
        return true;
    ...
    static public function AccessDecision($appName=MODULE_NAME) {
        ...
        if(self::checkAccess()) {
            ...
            if(empty($_SESSION[C('AGENT_ADMIN_AUTH_KEY')])) {
                if(C('AGENT_USER_AUTH_TYPE')==2) {
                    //加强验证和即时验证模式 更加安全 后台权限修改可以即时生效
                    //通过数据库进行访问检查
                    $accessList = self::getAccessList($_SESSION[C('AGENT_USER_AUTH_KEY')]);
                }else {
                    ...
                    $accessList = $_SESSION['_AGENT_ACCESS_LIST'];
                }
    ...
    static public function getAccessList($authId) {
        // Db方式权限数据
        $db     =   Db::getInstance(C('AGENT_RBAC_DB_DSN'));
        $table = array('role'=>C('AGENT_RBAC_ROLE_TABLE'),'user'=>C('AGENT_RBAC_USER_TABLE'),'access'=>C('AGENT_RBAC_ACCESS_TABLE'),'node'=>C('AGENT_RBAC_NODE_TABLE'));
	...
	static public function getModuleAccessList($authId,$module) {
        // Db方式
        $db     =   Db::getInstance(C('AGENT_RBAC_DB_DSN'));
        $table = array('role'=>C('AGENT_RBAC_ROLE_TABLE'),'user'=>C('AGENT_RBAC_USER_TABLE'),'access'=>C('AGENT_RBAC_ACCESS_TABLE'));
